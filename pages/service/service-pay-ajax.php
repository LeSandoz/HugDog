...
if (isset($_POST['act'])) {
    ...
    header("Refresh:3;url=./service-pay.php");
    exit("請正確操作!(3秒後返回)");
}
...
function sqlFunc($pdo, $tblName)
{
    ...
    // //是否議價
    // if ($serviceType != '') {
    //     $wherevar[] = "`sPayId` = '{$serviceType}' or ";
    // }
...
if ($action === 'serviceEdit') {
    ...
    $sql = "SELECT * FROM `service_pay` WHERE `id` = ?";
    $arrParam = [
        (int) $_POST['id']
    ];
    $stmt = $pdo->prepare($sql);
    ...
    ?>
        <form id="formEditData" method="POST">
            <div class="modal-header">
                <h5 class="modal-title" id="staticBackdropLabel">編輯付款方式</h5>
            ...
            <div class="modal-body px-5">
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="sPayId" class="form-control" value="<?php echo $arr['sPayId'] ?>" placeholder="請填入項目代號" maxlength="10" required>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="sPayName" class="form-control" value="<?php echo $arr['sPayName'] ?>" placeholder="請填入項目名稱" maxlength="10" required>
                    </div>
                </div>
                <div class="form-group row">
                    <label class="col-sm-2 col-form-label">項目說明</label>
                    <div class="col-sm-10">
                        <textarea type="text" name="sPayInfo" class="form-control" placeholder="請填入項目說明 (最多50字)" maxlength="50"><?php echo $arr['sPayInfo'] ?></textarea>
                    </div>
                </div>
                <div class="form-group row">
...
if ($action === 'serviceEditData') {
    //查詢項目名稱是否存在
    $sql = "SELECT * FROM `{$tblName}` WHERE `sPayId` =? AND `id`<>?";
    $arrParam = [
        str_replace(' ', '', $_POST['sPayId']),
        (int) $_POST['id']
    ];
    $stmt = $pdo->prepare($sql);
    $stmt->execute($arrParam);
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $sqlUpd = "UPDATE `{$tblName}` SET `sPayId`=?,`sPayName`=?,`sPayInfo`=?,`dataSts`=? 
        WHERE `id` = ?";
        $arrParam = [
            str_replace(' ', '', $_POST['sPayId']),
            $_POST['sPayName'],
...
if ($action === 'serviceAdd') {
    ...
    <form id="formAddData" method="POST">
        <div class="modal-header">
            <h5 class="modal-title" id="staticBackdropLabel">新增付款方式</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
        </div>
        <div class="modal-body px-5">
            <div class="form-group row">
                <label class="col-sm-2 col-form-label required">項目代號</label>
                <div class="col-sm-10">
                    <input type="text" name="sPayId" class="form-control" value="" placeholder="請填入項目代號" maxlength="10" required>
                </div>
            </div>
            <div class="form-group row">
                <label class="col-sm-2 col-form-label required">項目名稱</label>
                <div class="col-sm-10">
                    <input type="text" name="sPayName" class="form-control" value="" placeholder="請填入項目名稱" maxlength="10" required>
                </div>
            </div>
            <div class="form-group row">
                <label class="col-sm-2 col-form-label">項目說明</label>
                <div class="col-sm-10">
                    <textarea type="text" name="sPayInfo" class="form-control" placeholder="請填入項目說明 (最多50字)" maxlength="50"></textarea>
                </div>
            </div>
            <div class="form-group row">
                <label class="col-sm-2 col-form-label"></label>
...
if ($action === 'serviceAddData') {
    //查詢項目ID是否存在
    $sql = "SELECT * FROM `{$tblName}` WHERE `sPayId` =?";
    $arrParam = [
        str_replace(' ', '', $_POST['sPayId'])
    ];
    $stmt = $pdo->prepare($sql);
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $sqlIns = "INSERT INTO `{$tblName}` (`sPayId`,`sPayName`,`sPayInfo`,`dataSts`) VALUES (?,?,?,?)";
        $arrParam = [
            str_replace(' ', '', $_POST['sPayId']),
            $_POST['sPayName'],
            $_POST['sPayInfo'],
