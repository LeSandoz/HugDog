...
<head>
    ...
    $sqlTotal = "SELECT count(`mkId`) FROM `marketing_type`
    WHERE `mkId`= '$editmkId'";
                        ...
                        <form name="myForm" method="POST" action="marketing-deleteIds-mt.php">
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    <?php
                                    //SQL 敘述
                                    $sql = "SELECT `marketing_type`.`mtId`, `marketing_type`.`mkId`, `marketing_type`.`mtName`, `marketing_type`.`mtDiscount%`, `marketing_type`.`mtDiscount`, `marketing_type`.`pClass`,`marketing_type`.`created_at`, `marketing_type`.`updated_at`,`marketing`.`mkName`
                                FROM `marketing_type` INNER JOIN `marketing`
                                ON `marketing_type`.`mkId` = `marketing`.`mkId`
                                WHERE `marketing_type`.`mkId`= ?
                                ORDER BY `mtId` ASC 
                                LIMIT ?, ? ";

                                    //設定繫結值
                                    $arrParam = [$_GET['edit-mkId'], ($page - 1) * $numPerPage, $numPerPage];
                                    //查詢分頁後的學生資料
                            ...
                            <nav aria-label="Page navigation">
                                <ul class="pagination justify-content-center">
                                    <li class="page-item">
                                        <a class="page-link" href="<?= "marketing-mk.php?edit-mkId=" . $editmkId . "&page=" . $pagep ?>" aria-label="Previous">
                                            <span aria-hidden="true">&laquo;</span>
                                        </a>
                                    </li>
                                    <?php for ($i = 1; $i <= $totalPages; $i++) { ?>
                                        <li class="page-item"><a class="page-link" href="<?= "marketing-mk.php?edit-mkId=" . $editmkId . "&page=" . $i ?>"><?= $i ?></a></li>
                                    <?php } ?>
                                    <li class="page-item">
                                        <a class="page-link" href="<?= "marketing-mk.php?edit-mkId=" . $editmkId . "&page=" . $pagen ?>" aria-label="Next">
