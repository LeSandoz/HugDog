...
<form name="myForm" method="POST" action="./deleteCheck.php">
    <table class="border">
        ...
        <tbody>
        <?php
        $sqlOrder = "SELECT `orders`.`orderId`,`orders`.`created_at`,`orders`.`updated_at`, `payment_type`.`paymentTypeName`
                    FROM `orders` INNER JOIN `payment_type`
                    ON `orders`.`paymentTypeId` = `payment_type`.`paymentTypeId`
